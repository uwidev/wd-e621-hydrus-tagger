...
class WaifuDiffusionInterrogator:
    ...
    def load(
        self, cpu
    ) -> None:  # im pretty sure this just checks for the right onnx runtime package and downloads it if its missing, no clue why this cant be done on initial setup
        ...
        print("Using the following providers, preferring the first:")
        print(f"{providers=}")
    ...
    def interrogate(
        ...
        # find cutoff for general tags and character tags
        general_tags = dict(tags[self.tags.iloc[:, 2] == 0].values)
        character_tags = dict(tags[self.tags.iloc[:, 2] == 4].values)
